...
export function ChecklistTile() {
  ...
  /* the tick sound: two notes — a bright one when checking,
     a duller one when unchecking — so it reads as two events,
     not one toggle repeating */
  const tick = (on: boolean) =>
    on ? beep(880, 0.05, 0.09) : beep(520, 0.035, 0.09);

  /* Bubble tick for slider scrub — same recipe as the other
     tiles, throttled so raw range events don't saturate. */
  ...
  const copy = async () => {
    click();
    await navigator.clipboard.writeText(checklistSource);
    setCopied(true);
    setTimeout(() => setCopied(false), 1400);
  };

  /* Demo: fake cursor visits each unchecked row and clicks it,
     ending on the third — which triggers the whole "list falls
     on the floor" finish. Positions are read from real
     .chk-row buttons at play time so any Box/Corner tweaks
     stay tracked. */
  const play = useCallback(() => {
    const stage = stageRef.current;
    if (!stage || playing) return;
    const rows = Array.from(stage.querySelectorAll<HTMLButtonElement>(".chk-row"));
    ...
    const runStep = () => {
      if (step >= rows.length) {
        /* the list is already falling — hide the cursor and
           release the play lock; the 3s reset inside the
           component brings the tasks back on its own */
        setTimeout(() => {
      ...
      const x = rect.left - cardRect.left + 20;
